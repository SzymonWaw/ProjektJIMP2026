...
int is_planar(Edge *edges, int edge_count)
{
    ...
    if(n >= 3 && m > 3*n - 6)
    {
        printf("Graf NIE jest planarny (naruszony warunek Eulera: m > 3n - 6).\n");
        free_adjacency(adj, degree, n);
        return 0;
    }

    // Sprawdzenie spójności (DFS)
    int *visited = calloc(n + 1, sizeof(int));
    int *stack = malloc((n + 1) * sizeof(int));
    int top = 0;
    ...
    for(int i = 1; i <= n; i++)
    {
        if(!visited[i])
        {
            printf("Graf NIE jest planarny (niespojny).\n");
            free(visited);
            free(stack);
            free_adjacency(adj, degree, n);
            return 0;
    ...
    printf("Graf spelnia podstawowe warunki planarności.\n");
    return 1;
}
